...
//vvv=========================TEST CASES START=======================vvv	
	...
	@Test public void testValidCoins(){
		...
		for (Coin testCoin : testCoins){
			try{
				...
				assertEquals(testListener.ejectCoin(), testCoin);	//Coin has been received
				assertTrue(testListener.wasCoinAccepted());			//Coin is accepted 
				assertEquals(validSink.ejectCoin(), testCoin);		//Coin was accepted so should go to valid sink
				assertNotEquals(returnSink.ejectCoin(),testCoin);	//Coin should not get returned
			}
			catch(Exception e){
				fail(e.toString());
			}
	...
	@Test public void testInvalidCoins(){
		...
		for (Coin testCoin : testCoins){
			try{
				testSlot.addCoin(testCoin);
				assertEquals(testListener.ejectCoin(), testCoin);	//Coin has been received
				assertFalse(testListener.wasCoinAccepted());		//Coin is rejected 
				assertNotEquals(validSink.ejectCoin(), testCoin);	//Coin should not get go to valid sink
				assertEquals(returnSink.ejectCoin(), testCoin);		//Coin was rejected so should be returned
			}
			catch(Exception e){
				fail(e.toString());
			}
	...
	@Test public void testDisabledSlot(){
		...
		try{
			...
			assertNull(testListener.ejectCoin());		//No coin should be present to be ejected
			assertNull(validSink.ejectCoin());			//No coin should be present to be put in valid sink
			assertNull(returnSink.ejectCoin());			//No coin should be present to be put in return sink
	...
	@Test public void testValidSinkDisabled(){
		...
		try{
			...
			assertNull(returnSink.ejectCoin());					//No coin present in return sink
		}
		catch(Exception e){
			fail("Wrong exception thrown: " + e.toString());
		}
